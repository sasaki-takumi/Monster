<?php
	...
	echo $this->Form->hidden('location',array('value' => 'morioka'));
	...
	if (isset($result[0]['Monsteruser']['morioka'])) {
		echo '<div class="grow pic">'.$this->Html->image('rasetuki_morioka.png').'</div>'.'<br />';
	}else {
		echo '<div class="grow pic">'.$this->Html->image('m_rasetuki.png').'</div>'.'<br />';
	}
