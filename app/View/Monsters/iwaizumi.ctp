<?php
	...
	echo $this->Form->hidden('location',array('value' => 'iwaizumi'));
	...
	if (isset($result[0]['Monsteruser']['iwaizumi'])) {
		echo '<div class="grow pic">'.$this->Html->image('jaryuu_iwaizumi.png').'</div>'.'<br />';
	}else {
		echo '<div class="grow pic">'.$this->Html->image('jaryu.png').'</div>'.'<br />';
	}
