<?php
	...
	echo $this->Form->hidden('location',array('value' => 'miyako'));
	...
	if (isset($result[0]['Monsteruser']['miyako'])) {
		echo '<div class="grow pic">'.$this->Html->image('makuragaesi_miyako.png').'</div>'.'<br />';
	}else {
		echo '<div class="grow pic">'.$this->Html->image('m_makuragaesi.png').'</div>'.'<br />';
	}
