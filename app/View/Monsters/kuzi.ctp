<?php
	...
	echo $this->Form->hidden('location',array('value' => 'kuzi'));
	...
	if (isset($result[0]['Monsteruser']['kuzi'])) {
		echo '<div class="grow pic">'.$this->Html->image('kuzishi_mojabune.png').'</div>'.'<br />';
	}else {
		echo '<div class="grow pic">'.$this->Html->image('m_mojahune.png').'</div>'.'<br />';
	}
