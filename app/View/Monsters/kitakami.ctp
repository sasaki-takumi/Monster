<?php
	...
	echo $this->Form->hidden('location',array('value' => 'kitakami'));
	...
	if (isset($result[0]['Monsteruser']['kitakami'])) {
		echo '<div class="grow pic">'.$this->Html->image('kitakami_raiju.png').'</div>'.'<br />';
	}else {
		echo '<div class="grow pic">'.$this->Html->image('m_raiju.png').'</div>'.'<br />';
	}
