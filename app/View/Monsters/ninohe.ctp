<?php
	...
	echo $this->Form->hidden('location',array('value' => 'ninohe'));
	...
	echo "<h1>シタガラゴンボコ</h1><br />";

	if (isset($result[0]['Monsteruser']['ninohe'])) {
		echo '<div class="grow pic">'.$this->Html->image('ninohe_sitagarabonboko.png').'</div>'.'<br />';
	}else {
		echo '<div class="grow pic">'.$this->Html->image('m_tanuki.png').'</div>'.'<br />';
	}
